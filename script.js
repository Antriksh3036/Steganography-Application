...
(function () {
  ...
  // Scroll reaction: scrolling pushes the dust, drags the nodes at a fraction
  // of the scroll speed (depth), brightens the field and stretches specks into
  // short streaks. Set SCROLL_DUST, SCROLL_NODE, SCROLL_GLOW and SCROLL_STREAK
  // to 0 to turn it off.
  var SCROLL_DUST = 0.006; // how hard scrolling pushes the dust
  var SCROLL_NODE = 0.1; // nodes follow the scroll by this fraction
  var SCROLL_MAX = 60; // px/frame cap on the scroll speed that counts
  var SCROLL_GLOW = 0.5; // extra brightness at full scroll speed
  var SCROLL_STREAK = 0.35; // streak length per px/frame of scroll speed
  var SCROLL_STREAK_MAX = 14; // px cap on streak length
  ...
  function mount(host) {
    ...
    // Scroll tracking.
    var sdy = 0; // scroll distance accumulated since the last frame
    var scv = 0; // smoothed scroll speed, px per 60Hz frame (+ = scrolling down)
    var lastScroll = new WeakMap();
    ...
    function render(dt) {
      ...
      // Scroll speed, smoothed so the effect eases out after scrolling stops.
      if (k > 0) {
        scv += (clamp(sdy / k, -SCROLL_MAX, SCROLL_MAX) - scv) * 0.2;
        sdy = 0;
      }
      var scrolling = k > 0 && Math.abs(scv) > 0.2;
      var scrollK = SCROLL_DUST * k;
      var absScv = Math.abs(scv);
      var glow = 1 + SCROLL_GLOW * Math.min(absScv / 40, 1);
      var streak = Math.min(absScv * SCROLL_STREAK, SCROLL_STREAK_MAX);
      var streakTop = scv > 0 ? 0 : -streak; // trail behind the direction of travel
      ...
      for (i = 0; i < d.n; i++) {
        ...
        // Scrolling down sweeps dust upward (and vice versa), then it glides.
        if (scrolling) AY -= scv * scrollK;
        var VX = AX * damp;
        var VY = AY * damp;
        X += VX * k;
        Y += VY * k;
        ...
        var a = base[i] * (0.62 + 0.38 * Math.sin(t * rate[i] + phase[i])) * glow;
        b = (a * BUCKET_SCALE) | 0;
      ...
      for (g = 0; g < groups.length; g++) {
        ...
        for (j = 0; j < c; j++) {
          i = arr[j];
          var s = size[i];
          ctx.fillRect(dx[i] + dox, dy[i] + doy + streakTop, s, s + streak);
        }
      }
      ...
      for (i = 0; i < n.n; i++) {
        X = nx[i] + nvx[i] * k;
        Y = ny[i] + nvy[i] * k - scv * SCROLL_NODE * k;
        if (X < -MARGIN) X += spanW;
        else if (X > maxX) X -= spanW;
        if (Y < -MARGIN) Y += spanH;
        else if (Y > maxY) Y -= spanH;
    ...
    function onMotionChange() {
      ...
      if (reduced) {
        ...
        scv = sdy = 0; // and no scroll reaction
      }
    ...
    // Catches scrolling of the page AND of any scrollable container on it
    ...
    function onScroll(e) {
      ...
      var pos = isDoc ? window.pageYOffset || 0 : el.scrollTop;
      if (typeof pos !== "number") return;
      var key = isDoc ? window : el;
      var prev = lastScroll.get(key);
      lastScroll.set(key, pos);
      if (prev !== undefined) sdy += pos - prev;
    }
